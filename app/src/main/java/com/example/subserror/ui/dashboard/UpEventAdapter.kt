...
class UpEventAdapter : ListAdapter<EventEntity, UpEventAdapter.MyViewHolder>(
    DIFF_CALLBACK
) {

    class MyViewHolder(private val binding: UpcomingListLayoutBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(listEvent: EventEntity) {
            binding.tvUpcomingList.text = listEvent.name
            Glide.with(binding.root.context).load(listEvent.imageLogo).into(binding.imgUpcomingList)
        }
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MyViewHolder {
        val binding = UpcomingListLayoutBinding.inflate(LayoutInflater.from(parent.context), parent, false)
